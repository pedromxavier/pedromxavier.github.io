...
local function readyaml(fname, keys)
    local values = {
        href = fmt("/posts/%s/", getfilename(fname))
    }
...
local function makeitem(info)
    return pandoc.List {pandoc.Plain { -- This is a <li>
    pandoc.Link({pandoc.Span(format_title(info.title, info.subtitle), {
        class = "title"
    }), pandoc.Span(format_date(info.date), {
        class = "date"
    })}, info.href)}}
end
